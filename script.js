...
function drawNetwork() {
  ...
  particles.forEach((particle, index) => {
    ...
    context.fillStyle = 'rgba(74, 174, 255, .5)';
    context.fill();
    ...
    particles.slice(index + 1).forEach((other) => {
      ...
      context.strokeStyle = `rgba(48, 148, 226, ${(1 - distance / 115) * 0.13})`;
      context.stroke();
    });
  });
...
contactForm.addEventListener('submit', async (event) => {
  ...
  const message = [
    `${currentLanguage === 'en' ? 'Name' : 'Nombre'}: ${data.get('name')}`,
    `Email: ${data.get('email')}`,
    `${currentLanguage === 'en' ? 'Subject' : 'Asunto'}: ${data.get('subject')}`,
    '',
    data.get('message'),
  ].join('\n');
  ...
  try {
    ...
    setFormStatus('form.copied');
  } catch {
    setFormStatus('form.prepared');
  }
});
